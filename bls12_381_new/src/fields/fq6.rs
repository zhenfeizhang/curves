...
impl Fp6Parameters for Fq6Parameters {
    ...
    /// Multiply this element by the quadratic nonresidue 3 + 2*u.
    /// Make this generic.
    fn mul_fp2_by_nonresidue(fe: &Fq2) -> Fq2 {
        // (x+uy) * (3+2*u) = 3*x + u*3*y + u*2*x + u²*2*y = (3*x-10*y) + u*(2*x+3*y)
        let mut copy = *fe;
        let x_mul2 = copy.c0.double();
        let x_mul3 = x_mul2 + copy.c0;
        let y_mul3 = copy.c1.double() + copy.c1;
        let y_mul10 = copy.c1.double().double().double() + copy.c1.double();
        copy.c0 = x_mul3 - y_mul10;
        copy.c1 = x_mul2 + y_mul3;
        copy
    }
}
